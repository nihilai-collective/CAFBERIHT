...
static constexpr uint64_t cafberiht_width{ CAFBERIHT_WIDTH };
...
template<typename... bases> struct cafberiht : public bases... {
	static_assert(static_assert_printer_val<(sizeof...(bases) > 0), cafberiht_errors::empty_cafberiht_bases_pack>::impl);
	...
	template<uint64_t index_new> decltype(auto) get_core_by_index() const noexcept {
		static_assert(static_assert_printer_val<(index_new < size), cafberiht_errors::get_core_by_index_oob, index_new>::impl);
  ...
  protected:
	...
	constexpr void impl_internal_filtered([[maybe_unused]] arg_types&&... args) noexcept {
		if constexpr (mixin_type<base_type>::filter()) {
			static_assert(static_assert_printer_val<std::is_base_of_v<base_type, cafberiht>, cafberiht_errors::invalid_base_cast>::impl);
...
int main() {
	...
	cafberiht_val.get_core_by_index<23>();
